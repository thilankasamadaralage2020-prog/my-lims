...
# --- DATABASE SETUP ---
def init_db():
    conn = sqlite3.connect('lifecare_final_v53.db', check_same_thread=False)
    c = conn.cursor()
...
def get_fbc_structure(age_y, gender):
    ...
    if age_y < 5:
        ranges = {
            "Total White Cell Count (WBC)": "5000 - 13000", "Neutrophils": "25 - 45", "Lymphocytes": "45 - 65",
        ...
        ranges = {
            "Total White Cell Count (WBC)": "4000 - 11000", "Neutrophils": "40 - 75", "Lymphocytes": "20 - 45",
            "Monocytes": "02 - 10", "Eosinophils": "01 - 06", "Basophils": "00 - 01", "Hemoglobin (Hb)": "13.5 - 17.5" if gender == "Male" else "12.0 - 15.5", 
            "Red Blood Cell (RBC)": "4.5 - 5.5" if gender == "Male" else "3.8 - 4.8", "HCT / PCV": "40 - 52" if gender == "Male" else "36 - 47", 
...
def create_pdf(bill_row, results_dict=None, auth_user=None, is_report=False):
    ...
    if is_report:
        ...
        pdf.cell(70, 9, "  Component", 0, 0, 'L'); pdf.cell(30, 9, "Result", 0, 0, 'C')
        pdf.cell(30, 9, "Unit", 0, 0, 'C'); pdf.cell(60, 9, "Reference Range", 0, 1, 'C')
        ...
        for item in fbc_data:
            ...
            pdf.cell(70, 7, f"  {item['label']}", 0); pdf.cell(30, 7, str(res_val), 0, 0, 'C')
            pdf.cell(30, 7, item['unit'], 0, 0, 'C'); pdf.cell(60, 7, item['range'], 0, 1, 'C')
...
# --- MAIN APP (Rest of code remains same) ---
...
if not st.session_state.logged_in:
    ...
    # --- BILLING ---
        ...
        with st.container(border=True):
            ...
            final = total - disc
            st.info(f"Gross: {total:,.2f} | Discount: {disc:,.2f} | **Net Payable: {final:,.2f}**")
    ...
    # --- TECHNICIAN ---
        ...
        if 'last_rep' in st.session_state:
            ...
            st.download_button("📥 DOWNLOAD REPORT", create_pdf(rep_row, json.loads(rep_row['data']), rep_row['authorized_by'], True), f"Report_{rep_row['ref_no']}.pdf", use_container_width=True)
