...
# --- DATABASE SETUP ---
def init_db():
    conn = sqlite3.connect('lifecare_final_v52.db', check_same_thread=False)
    c = conn.cursor()
...
# --- UPDATED REFERENCE RANGES (BABY FBC SPECIALIZED) ---
def get_fbc_structure(age_y, gender):
    ...
    # Unit Setup
    units = {
        "Total White Cell Count (WBC)": "cells/cu/mm", "Neutrophils": "%", "Lymphocytes": "%", 
    ...
    if age_y < 5:
        # Specialized Baby FBC Ranges
        ranges = {
            "Total White Cell Count (WBC)": "5000 - 13000",
            "Neutrophils": "25 - 45",
            "Lymphocytes": "45 - 65",
            "Monocytes": "02 - 10",
            "Eosinophils": "01 - 06",
            "Basophils": "00 - 01",
            "Hemoglobin (Hb)": "10.5 - 14.0",
            "Red Blood Cell (RBC)": "3.8 - 5.2",
            "HCT / PCV": "32 - 42",
            "MCV": "75 - 90",
            "MCH": "24 - 30",
            "MCHC": "32 - 36",
        ...
        ranges = {
            "Total White Cell Count (WBC)": "4000 - 11000",
            "Neutrophils": "40 - 75",
            "Lymphocytes": "20 - 45",
            "Monocytes": "02 - 10",
            "Eosinophils": "01 - 06",
            "Basophils": "00 - 01",
            "Hemoglobin (Hb)": "13.5 - 17.5" if gender == "Male" else "12.0 - 15.5",
            "Red Blood Cell (RBC)": "4.5 - 5.5" if gender == "Male" else "3.8 - 4.8",
            "HCT / PCV": "40 - 52" if gender == "Male" else "36 - 47",
            "MCV": "80 - 100", "MCH": "27 - 32", "MCHC": "32 - 36", "RDW": "11.5 - 14.5",
...
if not st.session_state.logged_in:
    ...
    if st.session_state.user_role == "Admin":
        ...
        choice = st.sidebar.radio("Go to", ["User Management", "Doctors", "Tests"])
        if choice == "User Management":
            with st.form("u"):
                ...
                if st.form_submit_button("Add User"): 
                    c.execute("INSERT OR REPLACE INTO users VALUES (?,?,?)", (un, pw, rl)); conn.commit(); st.success("User Added")
            ...
            with st.form("t"):
                ...
                if st.form_submit_button("Add Test"):
                    c.execute("INSERT OR REPLACE INTO tests VALUES (?,?)", (tn, tp)); conn.commit(); st.success("Test Saved")
    ...
    # --- BILLING ---
        ...
        with st.container(border=True):
            ...
            selected = st.multiselect("Tests", [f"{r['test_name']} - {r['price']}" for _, r in tests_db.iterrows()])
            
            # Discount Logic
            gross = sum([float(s.split(" - ")[-1]) for s in selected])
            disc = st.number_input("Discount (LKR)", 0.0)
            final = gross - disc
            st.info(f"Gross: {gross:,.2f} | Discount: {disc:,.2f} | **Final: {final:,.2f}**")
            ...
            if st.button("SAVE BILL", use_container_width=True):
                if pname and selected:
                    ...
                    c.execute('''INSERT INTO billing (ref_no, salute, name, age_y, age_m, gender, mobile, doctor, tests, total, discount, final_amount, date, bill_user, status) 
                                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', (ref, sal, pname, ay, am, gen, mob, pdoc, tnames, gross, disc, final, str(date.today()), st.session_state.username, "Active"))
            ...
            with st.expander(f"📦 {row['ref_no']} - {row['name']} (Age: {row['age_y']}Y)"):
                ...
                with st.form(f"f_{row['ref_no']}"):
                    ...
                    for item in fbc_struct:
                        ...
                        res_data[item['label']] = col1.text_input(item['label'], key=f"i_{row['ref_no']}_{item['label']}")
                        col2.write(f"\n{item['unit']}")
                        col3.caption(f"Range: {item['range']}")
                    if st.form_submit_button("AUTHORIZE"):
                        c.execute("INSERT OR REPLACE INTO results VALUES (?,?,?,?,?)", (row['ref_no'], json.dumps(res_data), st.session_state.username, str(date.today()), "FBC"))
                        conn.commit(); st.session_state.last_rep = row['ref_no']; st.rerun()
        
        if 'last_rep' in st.session_state:
            rep_row = pd.read_sql_query(f"SELECT b.*, r.data, r.authorized_by FROM billing b JOIN results r ON b.ref_no = r.bill_ref WHERE b.ref_no='{st.session_state.last_rep}'", conn).iloc[0]
            st.download_button("📥 DOWNLOAD AUTHORIZED REPORT", create_pdf(rep_row, json.loads(rep_row['data']), rep_row['authorized_by'], True), f"Report_{rep_row['ref_no']}.pdf", use_container_width=True)
    ...
    # --- SATELLITE ---
        ...
        for _, r in reps.iterrows():
            with st.container(border=True):
                ...
                st.download_button("Print PDF", create_pdf(r, json.loads(r['data']), r['authorized_by'], True), f"Report_{r['ref_no']}.pdf", key=f"dl_{r['ref_no']}")
